...
class NeighborhoodHash(Kernel):
    ...
    def __init__(self, n_jobs=None, normalize=False, verbose=False, random_state=None, R=3, nh_type="simple", bits=8):
    ...
    def _vertex_sort(self, vertices, labels):
        ...
        if self._method_calling == 3:
            return (sorted(list(vertices), key=lambda x: float("inf") if labels[x] is None else labels[x]), labels)
        else:
